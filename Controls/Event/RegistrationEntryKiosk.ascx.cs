...
namespace RockWeb.Plugins.com_shepherdchurch.CustomCore.Event
{
    /// <summary>
    /// Block used to register for a registration instance.
    /// </summary>
    [DisplayName( "Registration Entry Kiosk" )]
    [Category( "Shepherd Church > Custom Core > Event" )]
    ...
    [LinkedPage( "Return Page", "The page to return to after completing a registration" )]
    public partial class RegistrationEntryKiosk : RockBlock
    {
        #region Fields
        ...
        private Person ActivePerson
        {
            get
            {
                if ( _activePerson == null )
                {
                    ...
                    {
                        _activePerson = new PersonService( new RockContext() ).Get( PageParameter( "Person" ).AsGuid() );
                    }
                }
        ...
        private PersonAlias ActivePersonAlias
        {
            get
            {
                return _activePerson != null ? _activePerson.PrimaryAlias : null;
            }
        }
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                if ( ActivePerson != null && ActivePerson.RecordTypeValue != null && personDv != null && ActivePerson.RecordTypeValue.Guid != personDv.Guid )
                {
            ...
            else
            {
                ...
                if ( Request.Form["__EVENTARGUMENT"] != null )
                {
                    ...
                    {
                        lbSummaryNext_Click( this, new EventArgs() );
                    }
                }
            }
                    ...
                    if ( gateway != null )
                    {
                        ...
                        if ( authorizedPersonAlias != null && authorizedPersonAlias.Person != null && paymentDetail != null )
                        {
                            if ( phCreateLogin.Visible )
                            {
                                ...
                                var mergeFields = Rock.Lava.LavaHelper.GetCommonMergeFields( this.RockPage, ActivePerson );
                                ...
                                {
                                    ExceptionLogService.LogException( ex, Context, this.RockPage.PageId, this.RockPage.Site.Id, ActivePersonAlias );
                                }
                            }
        ...
        private bool SetRegistrationState()
        {
            ...
            if ( registrationId.HasValue )
            {
                ...
                if ( ( registration.PersonAlias != null && registration.PersonAlias.PersonId == ActivePerson.Id ) ||
                    ( registration.CreatedByPersonAlias != null && registration.CreatedByPersonAlias.PersonId == ActivePerson.Id ) )
                {
            ...
            if ( RegistrationState == null && !string.IsNullOrWhiteSpace( registrationSlug ) )
            {
                ...
                if ( linkage != null )
                {
                    ...
                    GroupId = linkage.GroupId;
                    RegistrationState = new RegistrationInfo( ActivePerson );
                }
            }
        ...
        private void SetRegistrantState( int registrantCount )
        {
            if ( RegistrationState != null )
            {
                ...
                if ( RegistrationState.RegistrantCount == 0 && registrantCount == 1 && ActivePerson != null )
                {
                    var registrant = new RegistrantInfo( RegistrationInstanceState, ActivePerson );
                    if ( RegistrationTemplate.ShowCurrentFamilyMembers )
                    {
                                ...
                                f.FieldSource == RegistrationFieldSource.PersonField ) )
                        {
                            registrant.FieldValues.AddOrReplace( field.Id, 
                                new FieldValueObject( field, field.PersonFieldType == RegistrationPersonFieldType.FirstName ? ActivePerson.NickName : ActivePerson.LastName ) );
                        }
                    }
        ...
        private void ProcessPostSave( bool isNewRegistration, Registration registration, List<int> previousRegistrantIds, RockContext rockContext )
        {
            ...
            catch ( Exception postSaveEx )
            {
                ...
                ExceptionLogService.LogException( postSaveEx, Context, RockPage.PageId, RockPage.Layout.SiteId, ActivePersonAlias );
            }
        }
        ...
        private Registration SaveRegistration( RockContext rockContext, bool hasPayment )
        {
            ...
            if ( newRegistrar )
            {
                ...
                if ( ActivePerson != null && ActivePerson.FirstName == null )
                {
                    ActivePerson.FirstName = "";
                }

                if ( ActivePerson != null && ActivePerson.NickName == null )
                {
                    ActivePerson.NickName = ActivePerson.FirstName;
                }

                // If the 'your name' value equals the currently logged in person, use their person alias id
                if ( ActivePerson != null &&
                ( ActivePerson.NickName.Trim().Equals( registration.FirstName.Trim(), StringComparison.OrdinalIgnoreCase ) ||
                    ActivePerson.FirstName.Trim().Equals( registration.FirstName.Trim(), StringComparison.OrdinalIgnoreCase ) ) &&
                ActivePerson.LastName.Trim().Equals( registration.LastName.Trim(), StringComparison.OrdinalIgnoreCase ) )
                {
                    registrar = ActivePerson;
                    registration.PersonAliasId = ActivePerson.PrimaryAliasId;
                    ...
                    if ( !string.IsNullOrWhiteSpace( registration.ConfirmationEmail ) &&
                        !registration.ConfirmationEmail.Trim().Equals( ActivePerson.Email.Trim(), StringComparison.OrdinalIgnoreCase ) &&
                        ( !cbUpdateEmail.Visible || cbUpdateEmail.Checked ) )
                    {
                        var person = personService.Get( ActivePerson.Id );
                        if ( person != null )
                        {
                            ...
                            HistoryService.SaveChanges(
                                ...
                                person.Id,
                                personChanges, true, ActivePersonAlias != null ? ( int? ) ActivePersonAlias.Id : null );
                        }
                    }
                }
                else
            ...
            try
            {
                ...
                Task.Run( () =>
                    HistoryService.SaveChanges(
                        ...
                        registration.Id,
                        registrationChanges, true, ActivePersonAlias != null ? ( int? ) ActivePersonAlias.Id : null )
                );
                ...
                foreach ( var registrantInfo in RegistrationState.Registrants.ToList() )
                {
                    ...
                    Task.Run( () =>
                        HistoryService.SaveChanges(
                            ...
                            null, null, true, ActivePersonAlias != null ? ( int? ) ActivePersonAlias.Id : null )
                    );
        ...
        private bool ProcessPayment( RockContext rockContext, Registration registration, out string errorMessage )
        {
            ...
            else
            {
                if ( !string.IsNullOrWhiteSpace( hfSwipe.Value ) )
                {
                    ...
                    if ( registration.PersonAliasId.HasValue )
                    {
                        var registrar = new PersonAliasService( rockContext ).Queryable( "Person, Person.PhoneNumbers" ).Where( p => p.Id == registration.PersonAliasId ).FirstOrDefault();
                        paymentInfo.FirstName = registrar.Person.NickName;
                        paymentInfo.LastName = registrar.Person.LastName;

                        if ( registrar.Person.PhoneNumbers != null )
                        {
                            Guid homePhoneValueGuid = new Guid( Rock.SystemGuid.DefinedValue.PERSON_PHONE_TYPE_HOME );
                            var homephone = registrar.Person.PhoneNumbers.Where( p => p.NumberTypeValue.Guid == homePhoneValueGuid ).FirstOrDefault();
                            if ( homephone != null )
                            {
                                paymentInfo.Phone = homephone.NumberFormatted;
                            }
                        }

                        var homeLocation = registrar.Person.GetHomeLocation();
                ...
                if ( transaction.BatchId.HasValue )
                {
                    Task.Run( () =>
                        HistoryService.SaveChanges(
                            ...
                            batchChanges, true, ActivePersonAlias != null ? ( int? ) ActivePersonAlias.Id : null )
                    );
                    ...
                    Task.Run( () =>
                        HistoryService.SaveChanges(
                            ...
                            transaction.BatchId.Value,
                            txnChanges,
                            ActivePerson != null ? ActivePerson.FullName : string.Empty,
                            typeof( FinancialTransaction ),
                            transaction.Id, true, ActivePersonAlias != null ? ( int? ) ActivePersonAlias.Id : null )
                    );
                }
                ...
                else
                {
                    ...
                    if ( CurrentFormIndex == 0 && RegistrationState != null && RegistrationState.RegistrantCount > CurrentRegistrantIndex )
                    {
                        ...
                        if ( registrant.Id <= 0 && 
                            ...
                            RegistrationTemplate.ShowCurrentFamilyMembers && ActivePerson != null )
                        {
                            var familyMembers = ActivePerson.GetFamilyMembers( true )
                                .Select( m => m.Person )
                                .ToList();
        ...
        private void ShowSuccess( int registrationId )
        {
            ...
            try
            {
                    ...
                    registration.RegistrationInstance.RegistrationTemplate != null )
                {
                    ...
                    mergeFields.Add( "CurrentPerson", ActivePerson );
                    ...
                    {
                        lSuccessTitle.Text = template.SuccessTitle.ResolveMergeFields( mergeFields, ActivePerson );
                    }
                    else
                    {
                        lSuccessTitle.Text = "Congratulations";
                    ...
                    {
                        lSuccess.Text = template.SuccessText.ResolveMergeFields( mergeFields, ActivePerson );
                    }
                    else
                    {
                ...
                if ( nbAmountPaid.Visible = true &&
                    ...
                    string.IsNullOrWhiteSpace( hfSwipe.Value ) )
                {
                    ...
                    phCreateLogin.Visible = !new UserLoginService( rockContext ).GetByPersonId( ActivePerson != null ? ( int? ) ActivePerson.Id : null ).Any();
                }
                else
                {
                ...
                hfAutoReset.Value = LinkedPageUrl( "ReturnPage" );
            }
            catch ( Exception ex )
            {
                ExceptionLogService.LogException( ex, Context, this.RockPage.PageId, this.RockPage.Site.Id, ActivePersonAlias );
...
",
        ...
        private void CreatePersonField( RegistrationTemplateFormField field, bool setValue, object fieldValue, bool familyMemberSelected )
        {
            ...
            switch ( field.PersonFieldType )
            {
                ...
                case RegistrationPersonFieldType.Address:
                    {
                        var acAddress = new com.shepherdchurch.Misc.UI.Controls.MaskedAddressControl();
                        ...
                        if ( setValue && fieldValue != null )
                        {
                            ...
                            acAddress.SetUnmaskedValues( value );
                        }
                ...
                case RegistrationPersonFieldType.Email:
                    {
                        var tbEmail = new com.shepherdchurch.Misc.UI.Controls.MaskedEmailBox();
                        tbEmail.ID = "tbEmail";
                        tbEmail.Label = "Email";
                ...
                case RegistrationPersonFieldType.MobilePhone:
                    {
                        ...
                        if ( dv != null )
                        {
                            var ppMobile = new com.shepherdchurch.Misc.UI.Controls.MaskedPhoneNumberBox();
                ...
                case RegistrationPersonFieldType.HomePhone:
                    {
                        ...
                        if ( dv != null )
                        {
                            var ppHome = new com.shepherdchurch.Misc.UI.Controls.MaskedPhoneNumberBox();
                            ppHome.ID = "ppHome";
                ...
                case RegistrationPersonFieldType.WorkPhone:
                    {
                        ...
                        if ( dv != null )
                        {
                            var ppWork = new com.shepherdchurch.Misc.UI.Controls.MaskedPhoneNumberBox();
                            ppWork.ID = "ppWork";
                ...
                case RegistrationPersonFieldType.Address:
                    {
                        ...
                        var acAddress = phRegistrantControls.FindControl( "acAddress" ) as com.shepherdchurch.Misc.UI.Controls.MaskedAddressControl;
                        if ( acAddress != null )
                        {
                            acAddress.GetUnmaskedValues( location );
                            return location;
                        }
                        break;
                    }
        ...
        private void CreateSummaryControls( bool setValues )
        {
            ...
            if ( setValues && RegistrationState != null && RegistrationInstanceState != null )
            {
                ...
                else
                {
                    if ( ActivePerson != null )
                    {
                        tbYourFirstName.Text = ActivePerson.NickName;
                        tbYourLastName.Text = ActivePerson.LastName;
                        tbConfirmationEmail.Text = ActivePerson.Email;
                    }
                    else
                    {
                ...
                cbUpdateEmail.Visible = ActivePerson != null && !string.IsNullOrWhiteSpace( ActivePerson.Email );
        ...
        protected void lbSuccessDone_Click( object sender, EventArgs e )
        {
